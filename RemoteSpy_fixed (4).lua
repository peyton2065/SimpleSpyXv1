...
local CFG = {
    ...
    -- Set true to log ALL __namecall calls (yellow rows) to confirm the hook
    -- is firing.  Turn false once you see yellow rows appearing in-game.
    DEBUG_NAMECALL = true,
...
if not hookInstalled then
    local ok, err = pcall(function()
        ...
        hookInstalled    = true
        hookStrategyName = "per-instance-scan (" .. tostring(count) .. " remotes found)"
    end)
    if not ok then
...
local function addLogRow(idx, text)
    ...
    row.LayoutOrder     = idx
    row.Size            = UDim2.new(1, -8, 0, CFG.ROW_H)
    ...
    -- Debug [NC] rows get a gold background to stand out from real remote logs
    ...
    if not hasAutoCanvas then
        ...
        logScroll.CanvasSize = UDim2.new(0, 0, 0, manualCanvasH)
    end

    row.MouseButton1Click:Connect(function()
        if selectedIdx and entryFrames[selectedIdx] then
            local prev = entryFrames[selectedIdx]
            -- Restore original colour; don't restore debug colour
            if prev.Text:sub(1, 4) ~= "[NC]" then
                prev.BackgroundColor3 =
                    (selectedIdx % 2 == 0) and CFG.ENTRY_EVEN or CFG.ENTRY_ODD
            else
...
local btnClrLogs  = makeButton(ctrlPanel, "Clr Logs",      BGAP+(BW+BGAP)*5, 36, BW, BH, CFG.BTN_RED)
...
if hookInstalled then
    ...
    if CFG.DEBUG_NAMECALL then
        ...
        pushLog("[RemoteSpy] Set DEBUG_NAMECALL=false in CFG once working.")
    else
