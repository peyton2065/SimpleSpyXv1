...
-- Returns true if `instance` lives inside another player's hierarchy.
-- We skip hooking those remotes entirely -- only the local player's remotes
-- and shared game-service remotes are relevant.
...
if not hookInstalled then
    local ok, err = pcall(function()
        ...
        local oldFS = hookfunction(testRE.FireServer, cloneFn(function(s, ...)
            testFired = true
            return savedFS(s, ...)
        end))
    ...
    local ok, err = pcall(function()
        ...
        for _, desc in ipairs(game:GetDescendants()) do
            ...
            or cls == "BindableEvent" or cls == "BindableFunction" then
                -- Skip remotes that belong to other players' characters/GUIs
                if not isOtherPlayerDescendant(desc) then
...
-- Handles the new log format: "[ClassName] path  :method(args)"
-- and the old format:          "[ClassName] path(args)"
-- =============================================================================
local function generateCode(entry)
    if not entry then return "-- (no entry selected)" end

    -- New format: "[ClassName] Full.Path  :MethodName(args)"
    local cls, path, method, args =
        entry:match("^%[(.-)%] (.-)  :(.-)%((.*)%)%s*$")
    ...
    if not path then return "-- (could not parse entry)" end

    local cleanArgs   = (args and args:match("^%s*(.-)%s*$")) or ""
    local remoteName  = path:match("[^%.]+$") or path

    return 'local remote = game:FindFirstChild("' .. remoteName .. '", true)\n' .. '-- Adjust path if FindFirstChild returns the wrong instance\n' .. 'if remote then\n' .. '    remote:' .. (method or "FireServer") .. '(' .. cleanArgs .. ')\n' .. 'end'
end
...
local function addLogRow(idx, text)
    ...
    -- Colour by row type.
    -- [NC]     = debug namecall (gold)
    -- [Hooked] = startup inventory row (dim teal, non-selectable)
    -- [New]    = dynamically added remote (dim purple, non-selectable)
    -- normal   = actual fired remote (alternating dark rows, selectable)
    local prefix = text:sub(1, 6)
    local isInfo = (prefix == "[Hooke" or text:sub(1, 5) == "[New]" or text:sub(1, 4) == "[NC]")
    ...
    if not hasAutoCanvas then
        ...
        -- 1400 wide to match the fixed row width so horizontal scroll works
        logScroll.CanvasSize = UDim2.new(0, 1400, 0, manualCanvasH)
    else
        -- AutomaticCanvasSize handles height; force the width manually
        logScroll.CanvasSize = UDim2.new(0, 1400, 0, 0)
    end

    -- Info rows ([Hooked], [New], [NC]) are not selectable.
    -- Only actual fire-event rows (format "[Class] path  :method(args)") are selectable.
    if isInfo then return end

    row.MouseButton1Click:Connect(function()
        -- Deselect the previously selected row and restore its colour
...
-- Handles both new format "[Class] path  :method(args)" and
-- old format "[Class] path(args)".
...
btnRunCode.MouseButton1Click:Connect(function()
    local code = generateCode(getSelected())
    local fn, err = loadstring(code)
    if fn then pcall(fn)
    else pushLog("[RunCode error] " .. tostring(err)) end
end)
