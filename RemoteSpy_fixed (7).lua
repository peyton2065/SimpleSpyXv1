...
local function generateCode(entry)
    ...
    -- We have the path but not the method, so default to FireServer.
    local newCls, newPath = entry:match("^%[New%] %[(.-)%] (.+)$")
    if newPath then
        local remoteName = newPath:match("[^%.]+$") or newPath
        local method = (newCls == "RemoteFunction") and "InvokeServer"
    ...
    local cls, path, method, args = entry:match("^%[(.-)%] (.-)  :(.-)%((.*)%)%s*$")
