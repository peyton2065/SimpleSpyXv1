...
-- Optional 4th arg fromServer: when true, log line is prefixed with "[From] " (server -> client).
local function logRemote(self, methodName, args, fromServer)
    ...
    -- With optional prefix "[From] " for server->client (OnClientEvent/OnClientInvoke).
    local line = "[" .. cls .. "] " .. path .. "  :" .. methodName .. "(" .. formatArgs(args) .. ")"
    if fromServer then line = "[From] " .. line end
    pushLog(line)
end
    ...
    local ok, err = pcall(function()
        ...
        local function hookOne(remote)
            ...
            if cls == "RemoteEvent" or cls == "UnreliableRemoteEvent" then
                ...
                -- Incoming: server -> client (Ketamine-style)
                pcall(function()
            ...
            elseif cls == "RemoteFunction" then
                ...
                -- Incoming: server invokes client (Ketamine-style; requires getcallbackvalue)
                pcall(function()
                    local getcb = getfenv().getcallbackvalue
                    if not getcb then return end
                    local ok, cb = pcall(getcb, remote, "OnClientInvoke")
                    if not ok or type(cb) ~= "function" then return end
                    local origInv
                    origInv = hookfunction(cb, cloneFn(function(...)
        ...
        -- Helper: true if class is a remote we hook (Ketamine-style).
        ...
        -- Collect found remotes as a set to avoid double-counting when same instance
        -- appears in getnilinstances/getinstances and game:GetDescendants().
        local foundRemotes = {}
        local foundCount   = 0
        ...
        -- Ketamine-style: hook remotes from getnilinstances (nil-parented) first.
        local getNil = getfenv().getnilinstances
        if getNil then
            for _, v in ipairs(getNil() or {}) do
                if v and isRemoteClass(v.ClassName) and pcall(hookOne, v) then
                    addFound(v)
                end
            end
        end

        -- Ketamine-style: hook remotes from getinstances (all known instances) next.
        ...
        -- Hook every remote in the game tree (same instance may already be in foundRemotes).
        for _, desc in ipairs(game:GetDescendants()) do
            local cls = desc.ClassName
            if isRemoteClass(cls) then
...
local function generateCode(entry)
    ...
    if entry:sub(1, 8) == "[From] " then entry = entry:sub(9) end
    ...
    if newPath then
        ...
        if method == "OnClientEvent" then method = "FireServer" elseif method == "OnClientInvoke" then method = "InvokeServer" end
    end
...
local function addLogRow(idx, text)
    ...
    -- [New]    = dynamically added remote (dim purple, SELECTABLE)
    -- [From]   = incoming server->client (dim green, selectable)
    ...
    elseif text:sub(1, 6) == "[Hooke" then
        ...
        row.TextSize         = 11
    elseif text:sub(1, 8) == "[From] " then
        row.BackgroundColor3 = Color3.fromRGB(25, 50, 40)
    ...
    row.MouseButton1Click:Connect(function()
        if selectedIdx and entryFrames[selectedIdx] then
            ...
            elseif prevText:sub(1, 8) == "[From] " then
                prev.BackgroundColor3 = Color3.fromRGB(25, 50, 40)
...
-- Handles: [From] prefix, [New] rows, normal fire, old format.
local function extractPath(entry)
    if not entry then return nil end
    -- Strip incoming prefix so path is extracted correctly
    if entry:sub(1, 8) == "[From] " then entry = entry:sub(9) end
...
-- [From] rows use OnClientEvent/OnClientInvoke; we return FireServer/InvokeServer so Run re-sends to server.
local function parseLogEntry(entry)
    if not entry then return nil, nil, nil end
    local fromIncoming = entry:sub(1, 8) == "[From] "
    if fromIncoming then entry = entry:sub(9) end
    local path, method, args
    local newCls, newPath = entry:match("^%[New%] %[(.-)%] (.+)$")
    if newPath then
        path = newPath
        ...
        -- Incoming rows: map to server methods so Run Code re-sends to server
        if fromIncoming and path then
            if method == "OnClientEvent" then method = "FireServer"
            elseif method == "OnClientInvoke" then method = "InvokeServer"
            end
        end
    end
    if path then
